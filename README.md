...
> ⚠️ 默认口令仅用于首次登录，请登录后立即修改；当前版本密码为明文存储，建议改造为加盐哈希。
...
| 限制 | 说明 |
|---|---|
| 密码存储 | 明文存储，未使用 SHA256 / 加盐哈希 |
| 人脸阈值 | 相似度阈值硬编码 0.75，无调参界面 |
| 员工注册 / 修改页 | 无摄像头实时预览 |
| Linux 端 | 源码存在，未验证最新版本 |

**改进方向**

1. 密码加盐哈希（SHA256 + Salt）存储
2. 人脸识别阈值可视化调参
3. 员工注册时增加摄像头实时预览
4. 集成口罩人脸识别模型
5. 增加远程 Web 管理端
...
|---|---|
| v1.0 | 基础打卡功能 |
| v2.0 | 人脸识别模块集成（SeetaFace6 Windows / Linux 双平台） |
| v3.0 | 人脸识别完整集成 + 工作线程优化 + 考勤规则 + bug 修复 |
| v4.0 | 权限分级 + 单文件打包（Enigma Virtual Box），完整功能版本 |
